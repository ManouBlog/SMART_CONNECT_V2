...
export default createStore({
  ...
  mutations: {
    addExperiences(state, person) {
      axios
        .post(
          "http://192.168.1.14:8000/api/postNewExperience",
          {
  ...
  actions: {
    ...
    get_contrats({ commit,state }) {
      commit('TOOGLESPINNER',true)
      axios
        .get("http://192.168.1.14:8000/api/admin/allContrats", {
          headers: {
            Authorization: "Bearer " + state.token,
          },
        })
     ...
     async getInfoUser({commit,state}) {
      if(state.token){
       await axios.get("http://192.168.1.14:8000/api/voirInfoUserConnect", {
          headers: {
            Authorization: "Bearer " + state.token,
          },
        })
