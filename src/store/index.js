...
export default createStore({
  ...
  mutations: {
    addExperiences(state, person) {
      axios
        .post(
          "https://backend-test.monbrobroli.com/api/postNewExperience",
          {
  ...
  actions: {
    get_users({ commit,state }) {
      ...
      axios
        .get("https://backend-test.monbrobroli.com/api/list_entreprise", {
          headers: {
            Authorization: "Bearer " + state.token,
          },
        })
    ...
    get_contrats({ commit,state }) {
      commit('TOOGLESPINNER',true)
      axios
        .get("https://backend-test.monbrobroli.com/api/admin/allContrats", {
          headers: {
            Authorization: "Bearer " + state.token,
          },
        })
     ...
     async getInfoUser({commit,state}) {
      if(state.token){
       await axios.get("https://backend-test.monbrobroli.com/api/voirInfoUserConnect", {
          headers: {
            Authorization: "Bearer " + state.token,
          },
        })
