...
<template>
  <section class="myconteneur">
    <div class="jobs_filters">
      <div class="container-fluid">
        <form>
          <div>
            <VueMultiselect v-model="categorie" :options="list_categorie" :multiple="true" :taggable="true"
              :tag="addTag" label="categorie" track-by="categorie" :placeholder="texte">
            </VueMultiselect>

          </div>
          <div>
            <VueMultiselect v-model="selectedCountries" :options="list_countries" :multiple="true" label="label"
              track-by="id" placeholder="Sélectionnez plusieurs pays" />
          </div>
          <div class="position-relative">
            <input type="text" class="form-control" style="min-height: 40px !important;" :placeholder="texte1"
              v-model="searchName" />
          </div>

          <div class="position-relative">
            <input type="text" class="form-control" style="min-height: 40px !important;" :placeholder="texte2"
              v-model="searchLieu" />
          </div>
          <div>
          ...
          <div class="conteneur_btn_search">
            <button class="btn-search btn" :disabled="!isFormValid" @click.prevent="searchOffres">
              Rechercher
            </button>
          </div>
        </form>
            ...
            <section class="one conteneur_ecriteau">
              ...
              <div class="informations_offres">
                <span class="badge bg-danger badge_new"
                  :class="configUtils.showJobNew(offre.created_at) ? null : 'd-none'">{{ texte5 }}</span>
  ...
  ">
                ...
                <h1 style="font-size: 0.9em; margin: 0.5em 0"><span style="color:orange">{{
                  offre?.competence?.categorie?.categorie }}</span></h1>
            ...
            <div style="position: absolute;bottom: 0.5em;width: 90%;display: flex;place-content: center;">
              <router-link class="see_detail" :to="{ name: 'OffreDetail', params: { id: offre.id } }">
                <button class="btn btn-Conex mt-3" style="padding:0.4em;width: 200px;">
        ...
        <div v-if="!list_offre.length"
          ...
          <img src="/notOffers.png" alt="Aucune offre disponible" class="img-fluid" style="max-width: 350px" />
        </div>
      </div>
    </div>
    <div class="d-flex justify-content-center my-4" v-if="MylistOffre.length > 0">
      <n-pagination v-model:page="currentPage" :page-size="pageSize" :item-count="MylistOffre.length" show-size-picker
        :page-sizes="[5, 10, 20]" @update:page="currentPage = $event" @update:page-size="
          (size) => {
            pageSize = size;
            currentPage = 1;
          }
