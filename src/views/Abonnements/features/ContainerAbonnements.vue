...
<template>
  ...
  <div v-if="tabsSubAbonnement.length">
    <div style="display: flex;justify-content: center;">
      ...
      <n-tabs v-else v-model:value="select_mode_payment_tab" type="segment" @update:value="handleSelect_mode_Payement"
        style="margin:1em 0;max-width: 300px;">
        <n-tab-pane v-for="tab in Array.from(new Map(
    ...
    <SubAbonnementsArtisans v-if="select_mode_payment_tab === 'Artisan'" :abonnements="subAbonnement"
      :type_abonnements="'Artisan'" />
    <SubAbonnementsVeteran v-if="select_mode_payment_tab === 'Vétéran Hors Grade'" :abonnements="subAbonnement"
      :type_abonnements="'Vétéran Hors Grade'" />
    <SubAbonnementsVeteran v-if="select_mode_payment_tab === 'Vétéran Senior'" :abonnements="subAbonnement"
      :type_abonnements="'Vétéran Senior'" />
    <SubAbonnementsVeteran v-if="select_mode_payment_tab === 'Vétéran'" :abonnements="subAbonnement"
  ...
  <div v-if="!tabsSubAbonnement.length">
    <section v-if="!storeAbonnement.addProfilHybride.length"
      ...
      <div v-for="item in abonnements.filter(
      ...
      )" :key="item.id" :class="item?.libelle == 'BROBROLI PRO' || item.libelle === 'BROBROLI HOME' || item?.libelle == 'BROBROLI'
          ? 'color_brobroli_pro'
        ...
        <div :style="{
          ...
          <p v-if="['BROBROLI+', 'BROBROLI PRO MAX', 'BROBROLI HOME MAX'].includes(item.libelle)"
        ...
        <p class="shadow-sm small" style="font-weight: bold;
     text-align:center;
    ...
    " :style="{
      ...
      padding: item.libelle === 'BROBROLI' ? '0.9em' : '0.6em',
        ...
        <div
          v-if="item?.categorie && ['Etudiant', 'Particulier', 'Artisan', 'Professionnel'].some(role => role === item?.categorie?.categorie)">
          <contentAbonnement :item="item" :elmentsOfBtn="elmentsOfBtn" :type_abonnements="type_abonnements" />
        </div>
        <section v-else>
          ...
          <div class="conteneur-btn">
            <Buttons :isDisabled="storeAbonnementUser?.planAbonnement?.abonement_id == item.id"
              :elmentsOfBtn="elmentsOfBtn" :shapeBtn="'round'" @created="handleCreateAbonnement(item)" />
          </div>
        </section>
      </div>
    </section>
    <section v-if="storeAbonnement.addProfilHybride.length"
      ...
      <div v-if="formuleAbonnementOfUserConnected">
        ...
        ">
          ...
          <div
            v-if="item?.categorie && ['Etudiant', 'Particulier', 'Artisan', 'Professionnel'].some(role => role === item?.categorie?.categorie)">
            <ContentAbonnementForAddProfilHybride :item="item" :elmentsOfBtn="elmentsOfBtn"
              :type_abonnements="type_abonnements" />
          </div>
          <section v-else>
            <p style="text-align:center;position: absolute;right: 0px;top:0px;">
              <span v-if="storeAbonnementUser?.planAbonnement?.abonement_id === item.id"
                style="background-color: green;" class="badge">
                Active
              </span>
            </p>
            <div class="d-flex align-items-center gap-5 justify-content-center main-color">
      ...
      <div v-if="!formuleAbonnementOfUserConnected">
        ...
        ">
          ...
          <div
            v-if="item?.categorie && ['Etudiant', 'Particulier', 'Artisan', 'Professionnel', 'Entreprise'].some(role => role === item?.categorie?.categorie)">
            <ContentAbonnementForAddProfilHybride :item="item" :elmentsOfBtn="elmentsOfBtn"
              :type_abonnements="type_abonnements" />
          </div>
          <section v-else>
