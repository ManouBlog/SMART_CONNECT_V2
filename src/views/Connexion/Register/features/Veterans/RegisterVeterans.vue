...
<template>
  ...
  <a-steps :style="{
    ...
    <a-step title="Profil Hybride" description="Ajoutez votre profil hybride" />
  ...
  <a-form layout="vertical" :model="formState" @finish="onFinish" @finishFailed="onHandleFailed">
      ...
      <div>
        ...
        <div class="round-container">
          <label v-for="item in allAnwserProfilHybride" :key="item.value" class="round-item">
            <input type="radio" name="profilHybride" :value="item.value"
              :checked="formState.optionsAnswer === item.value" v-model="formState.optionsAnswer"
    ...
    <div v-show="currentStep === 3">
      <a-row :gutter="[16, 24]">
        ...
        <a-col :xs="24" :md="12">
          <a-form-item :label="'Traitement préférentiel'">
            <a-select style="width: 100%;" v-model:value="formState.treatment_preferentiel"
              placeholder="Sélectionnez votre traitement préférentiel" show-search option-filter-prop="label">
              <a-select-option v-for="item in valueExpertise" :key="item.value" :value="item.value" :label="item.label">
                {{ item.label }}
              </a-select-option>
            </a-select>
          </a-form-item>
    ...
    <div v-show="currentStep === 4">
      <a-row :gutter="[16, 24]">
        ...
        <a-col :xs="24" :md="12">
          <a-form-item label="Domaine" name="filiere" :rules="[{ required: true, message: 'Ajoutez votre domaine' }]">
            <a-input v-model:value="formState.filiere" placeholder="Domaine" />
          </a-form-item>
        </a-col>
        <a-col :xs="24" :md="12">
          <a-form-item :label="'Statut professionnel'" :rules="[{ required: true, message: 'Séléctionnez un statut' }]">
            <a-select style="width: 100%;" v-model:value="formState.statut_talent"
              placeholder="Sélectionnez votre Niveau de carrière" show-search option-filter-prop="label">
              <a-select-option v-for="item in StatutVeterans" :key="item.value" :value="item.value" :label="item.label">
                {{ item.label }}
              </a-select-option>
            </a-select>
            <span v-if="formState.statut_talent == 'Vétéran Hors Grade'" style="padding:1em;color: red;">Nous activons
              le
        ...
        <a-col :xs="24" :md="12" v-if="formState.statut_talent !== 'Vétéran Hors Grade'">
          <a-form-item label="Voulez-vous activer le mode discret ?" name="mode_discret">
            <a-tooltip title="Mode discret : Masquer votre profil des recherches publiques. 
Les entreprises ne pourront pas voir votre profil mais vous voyez leurs offres et pouvez postuler." placement="right">
              <template #title>
                <span>
              ...
              <a-switch v-model:checked="formState.mode_discret" checked-children="Actif" un-checked-children="Inactif"
                size="default" class="switch-green" />
            </a-tooltip>
          </a-form-item>
        </a-col>
        <a-col :xs="24" :md="24">
      ...
      <a-row :gutter="[16, 24]">
        <a-col :xs="24" :md="12">
          <a-form-item :label="'Mode de travail'"
            ...
            <a-select style="width: 100%;" v-model:value="formState.modeTravail"
              placeholder="Sélectionnez votre mode de travail" show-search option-filter-prop="label">
              <a-select-option v-for="item in valueModeDeTravail" :key="item.value" :value="item.value"
                :label="item.label">
                {{ item.label }}
              </a-select-option>
            </a-select>
        ...
        <a-col :xs="24" :md="12">
          <a-form-item :label="'Temps de travail'"
            :rules="[{ required: true, message: 'Séléctionnez un temps de travail' }]">
            <a-select style="width: 100%;" v-model:value="formState.tempsTravail"
              placeholder="Sélectionnez votre Niveau de carrière" show-search option-filter-prop="label">
              <a-select-option v-for="item in valueTempsTravail" :key="item.value" :value="item.value"
                :label="item.label">
                {{ item.label }}
              </a-select-option>
            </a-select>
    ...
    <div v-show="currentStep === 6">
      <a-row :gutter="[16, 24]">
        ...
        <a-col :xs="24" :md="12">
          <a-form-item name="upload" :label="texte9">
      ...
      <!-- <div>
        ...
        <div class="round-container">
          <label v-for="item in allAnwserForAssitance" :key="item.value" class="round-item">
            <input :disabled="loading" type="radio" name="profilHybride" :value="item.value"
              :checked="formState.answerAssistance === item.value" @change="formState.answerAssistance = item.value" />
            <span class="round-label">
              {{ item.label }}
            </span>
          </label>
