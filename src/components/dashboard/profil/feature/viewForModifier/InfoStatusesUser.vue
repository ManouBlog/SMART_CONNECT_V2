...
<template>
  ...
  <n-modal v-model:show="showModalDeleteMyProfilHybride" style="width:80%; height: 400px; 
    overflow-y: auto; 
    max-height: 80vh;" preset="card" :closable="false">
    <template #header>
  ...
  <n-modal v-model:show="showModalAddProfilHybride" style="width:80%; height: 400px; 
    overflow-y: auto; 
    max-height: 80vh;" preset="card" :closable="false" @after-leave="resetProfilHybrideData">
    <template #header>
      <div class="modal-header">
    ...
    <AddProfilHybride :ProfilsUser="profils" />

  </n-modal>

  <n-modal v-model:show="showModalChangeProfilOfBase" style="width:80%; height: 400px; 
    overflow-y: auto; 
    max-height: 80vh;" preset="card" :closable="false" @after-leave="resetData">
    <template #header>
      <div class="modal-header">
    ...
    <div v-if="!isLoading">
      <section v-if="allStatuses.length">
        <div class="w-100 mb-4">
          ...
          <select name="status" id="statusSelect" v-model="selectedStatus"
            class="form-select rounded-4 shadow-sm border-0 w-100" style="min-height: 50px"
            :disabled="!allStatuses.length" @change="handleStatutProfil" required>
            <!-- Option placeholder -->
            ...
            <!-- Options dynamiques -->
            <option v-for="value in allStatuses" :key="value.id" :value="JSON.stringify(value)">
              {{ value.statut }}
            </option>
          </select>
        </div>
        ...
        <transition name="fade-slide">
          <div
            v-if="selectedParseStatus && ['Particulier', 'Vétéran', 'Etudiant', 'Professionnel'].includes(selectedParseStatus.statut)">
            <label style="color: rgba(0, 0, 0, 0.88); font-size: 14px;">
              Souhaitez-vous adopter un profil hybride ?
            </label>

            <div class="round-container">
              <label v-for="item in allProfilHybrideAnswer" :key="item.value" class="round-item">
                <input type="radio" name="profilHybride" :value="item.value" v-model="optionsAnswer" />
        ...
        <transition name="fade-slide">
          <div
            v-if="optionsAnswer === 'oui' && ['Particulier', 'Vétéran', 'Professionnel'].includes(selectedParseStatus.statut)">
            <label style="color: rgba(0, 0, 0, 0.88); font-size: 14px;">
              Profils disponibles
            </label>
            <div class="round-container">
              <label v-for="item in allProfilsHybrides" :key="item.id" class="round-item">
                <input type="checkbox" :value="item" v-model="profilHybride" />
                <span class="round-label">
                  {{ item.statut }}
                </span>
              </label>
        ...
        <div v-if="optionsAnswer || selectedParseStatus">
          <FieldsVeteran :profilHybride="profilHybride" :optionsAnswer="optionsAnswer"
            :profilOfAbonnement="selectedParseStatus" v-if="selectedParseStatus.statut === 'Vétéran'" />
          <FieldsArtisan :profilOfAbonnement="selectedParseStatus" v-if="selectedParseStatus.statut === 'Artisan'" />
          <FieldsProfessionnel :profilHybride="profilHybride" :optionsAnswer="optionsAnswer"
            :profilOfAbonnement="selectedParseStatus" v-if="selectedParseStatus.statut === 'Professionnel'" />
          <FieldsCompany v-if="selectedParseStatus.statut === 'Entreprise'" :profilOfAbonnement="selectedParseStatus" />
        </div>
      </section>
  ...
  <a-card style="
    ...
    <div style="padding: 0 1em;" v-if="
      (
        profils?.user?.abonement?.some(
          item =>
            item.statut === 'success' &&
            item.abonement.libelle === 'BROBROLI+'
        ) &&
        profils?.user?.statut?.statut === 'Professionnel'
      ) ||
      profils?.user?.statut?.statut === 'Vétéran'
    ">
      <label class="ant-form-item-label" style="display:flex;flex-direction: column;">
        <span style="font-size:1em;">Mode Discret</span>
      </label>
      <a-switch v-model:checked="modeActif" checked-children="Actif" un-checked-children="Inactif"
        @change="handleModeChange" size="default" class="switch-green" />
    </div>
    ...
    <div class="p-4">
      ...
      <div v-else :style="{
        ...
        <div v-for="(profil, index) in profils?.user?.statuses" :key="index" style="
          ...
          <p v-if="profils?.user?.statut?.statut == profil.statut && profils?.user?.statut?.statut.includes('Vétéran')">
            ({{ profils?.statut_talent }})</p>
          <p
            v-if="profils?.user?.statut?.statut == profil.statut && profils?.user?.statut?.statut.includes('Artisan')">
            ({{ profils?.statut_talent }})</p>
          <span v-if="profils?.user?.statut?.statut == profil.statut" class="badge"
