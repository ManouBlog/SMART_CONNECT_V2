    ...
    <div v-if="!spinnerModify">
      <form v-if="offre_id && this.$store.state.user.user.statut.statut === 'Entreprise'" class="container">
        ...
        <div class="row">
          ...
          <div class="text-left my-3 col-md-6">
            <label>{{ texte8 }}</label>
            <input class="form-control" type="text" v-model="offre_id.salaire" placeholder="ex:35000" required />
          </div>
        </div>
        <div class="row">
          <div class="text-left my-3 col-md-6">
          ...
          <div class="text-left my-3 col-md-6">
            <label>{{ texte11 }}</label>
            <input class="form-control" type="number" v-model="offre_id.nbre_person" placeholder="ex:5 ou 10" required
              min="1" />
          </div>
        </div>
        <div class="row">
          <div class="text-left my-3 col-md-6">
            <label>{{ texte12 }}</label>
            <input class="form-control" type="text" id="lieu" v-model="offre_id.lieu" placeholder="ex:Angré" required />
          </div>
          <div class="text-left my-3 col-md-6">
            <label>{{ texte13 }}</label>
            <input class="form-control" type="datetime-local" id="date_time" v-model="offre_id.debut"
              :min="new Date().toJSON().slice(0, 16)" required />
          </div>
        </div>
        <div class="row">
          <div class="text-left my-3 col-md-6">
            <label>{{ texte14 }}</label>
            <input class="form-control" type="datetime-local" id="date_fin" v-model="offre_id.fin" required />
          </div>
          <div class="text-left my-3 col-md-6">
            <label>{{ texte15 }}</label>
            <input class="form-control" type="date" v-model="offre_id.job_debut" required id="date_job_debut" />
          </div>
        </div>
        <div class="row">
          <div class="text-left my-3 col-md-6">
            <label>{{ texte16 }}</label>
            <input class="form-control" type="date" v-model="offre_id.job_fin" />
          </div>
        ...
        <div class="row">
          <div>
            <div class="text-left my-3 col-md-6" v-if="this.$store.state.user.user.statut.statut === 'Entreprise'">
              <label>Choisir un mode de travail</label>
              <VueMultiselect v-model="offre_mode_travail" :options="[{ value: 'Présentiel', label: 'Présentiel' },
              { value: 'Télétravail', label: 'Télétravail' },
              { value: 'Hybride', label: 'Hybride' }]" label="label" track-by="label" />
            </div>
        ...
        <div style="display:flex;justify-content:center;">
          <button :disabled="isLoading" class="btn p-5 mt-4 btn-designer fw-bold bg-warning"
...
>
  Relancer
        ...
        <a-form :model="formState" :rules="rules" layout="vertical" @finish="update_mission">

          <a-row :gutter="[16, 16]">
            <!-- Catégorie -->
            <a-col :xs="24" :md="12">
              <a-form-item name="categorie_offre_id" :label="texte1">
                <a-select v-model:value="formState.categorie_offre_id" @change="selectCategorieFormState"
                  name="categorie_offre_id" placeholder="texte2" show-search :options="categoriesOffres"
                  :filter-option="filterOption" disabled option-filter-prop="label"
                  :field-names="{ label: 'categorie', value: 'id' }">
                  <!-- options dynamiques -->
                  <a-select-option v-for="item in categoriesOffres" :key="item.id" :value="item.id">
                    {{ item.categorie }}
                  </a-select-option>
              ...
              <div v-if="formState.categorie_offre_id === 'autre'" style="margin:0.5em 0">
                <a-form-item name="otherDomaine" label="Autre domaine">
                  <a-input v-model:value="formState.otherDomaine"
            ...
            <!-- Compétence -->
            <a-col :xs="24" :md="12">
              <a-form-item v-if="formState.categorie_offre_id !== 'autre'" name="competence_id" :label="texte3">
                <a-select v-model:value="formState.competence_id" name="competence_id"
                  @change="chooseCompetenceFormState" disabled>
                  <a-select-option v-for="(item, index) in competenceWithCategorie" :key="index" :value="item.id">
                    {{ item.competence }}
                  </a-select-option>
              ...
              " style="margin:0.5em 0">
                <a-form-item name="otherPoste" label="Autre poste">
                  <a-input v-model:value="formState.otherPoste"
                    style="height:30px !important;border:1px solid #cdcccc !important" />
                </a-form-item>
              </div>
            </a-col>
          ...
          <a-row :gutter="[16, 16]" class="my-5">
            <a-col :span="24" style="text-align: center">
              <button class="btn btn-warning btn-designer" type="submit" :disabled="isDisabled">
                Modifier
              </button>
            </a-col>
          </a-row>
