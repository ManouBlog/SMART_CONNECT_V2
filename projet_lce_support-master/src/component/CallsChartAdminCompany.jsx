...
export default function CallsChartAdminCompany() {
...
const [selectedPeriod, setSelectedPeriod] = useState("annee");
  ...
  // Algorithme ISO : date du jeudi de la semaine (stable)
  const simple = new Date(year, 0, 1 + (weekNum - 1) * 7);
  const dow = simple.getDay(); // 0=dim,1=lun,...,6=sam
  let monday;

  if (dow <= 4) {
    monday = new Date(simple.getTime() - (dow - 1) * 86400000); // Retrait vers lundi
  } else {
    monday = new Date(simple.getTime() + (8 - dow) * 86400000); // Avance vers lundi suivant
  }

  const sunday = new Date(monday.getTime() + 6 * 86400000); // +6 jours
  ...
  return {
    start: formatDate(monday), // Lundi YYYY-MM-DD
    end: formatDate(sunday)  ,
    periode:1  // Dimanche YYYY-MM-DD
  };
}
...
const now = new Date();  
const rangeWeek = selectedPeriod === "semaine" ? getWeekRange(selectedDate):null;
//console.log('rangeWeek',rangeWeek)
const periodConfig = {
  ...
  'semaine': { type: 'week', value: rangeWeek }, // Numéro semaine actuelle
...
const config = periodConfig[selectedPeriod];
  ...
  fetchStatistique({
    ...
    dataSend: { 
     [config.type]: filterType !== "week" ? config.value:null,  
     period:filterType === "week" ? config.value.periode:null,
     start_date:filterType === "week" ? config.value.start:null,
     end_date:filterType === "week" ? config.value.end:null,
     type:filterType === "week" ? 'period' : filterType 
    },
  })
);
      ...
      chart
        ...
        .encode("y", "open")
        .encode("color", () => "ouvert")
        .encode("series", () => "ouvert")
        
        .axis("y", { title: null })
        .scale("y", { nice: true })
        .scale("color", {
          domain: ["total", "fermés","ouvert", "taux de fermeture", "résolus","rejetés"],
          range: ["#4f39f6","green","#2b7fff", "#2ca02c", "#016739","#ca0101"],
        });
        ...
        .encode("x", "label")
        .encode("y", "resolved")
        .encode("color", () => "résolus")
        .encode("series", () => "résolus")
        .axis("y", { title: null })
        .scale("y", { nice: true });
        ...
        .encode("x", "label")
        .encode("y", "rejected")
        .encode("color", () => "rejetés")
        .encode("series", () => "rejetés")
        .axis("y", { title: null })
        .scale("y", { nice: true });
      ...
      chart
        ...
        .encode("x", "label")
        .encode("y", "closed")
        .encode("color", () => "taux de fermeture")
...
}, [selectedDate, dispatch, user?.role_id]);
